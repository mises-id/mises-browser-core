...
import org.chromium.chrome.browser.toolbar.bottom.MisesBottomControlsCoordinator;
import org.chromium.chrome.browser.toolbar.bottom.MisesScrollingBottomViewResourceFrameLayout;
...
import org.chromium.chrome.browser.toolbar.top.MisesTopToolbarCoordinator;
...
public class MisesToolbarManager extends ToolbarManager {
    private static final String TAG = "MisesToolbarManager";
    ...
    private HomepageManager.HomepageStateListener mMisesHomepageStateListener;
    private AppCompatActivity mActivity;
    private WindowAndroid mWindowAndroid;
    private CompositorViewHolder mCompositorViewHolder;
    private final Object mLock = new Object();
    private boolean mBottomControlsEnabled;
    private MisesScrollingBottomViewResourceFrameLayout mBottomControls;
    ...
    public MisesToolbarManager(
        ...
        mMisesHomepageStateListener =
                () -> {
                    if (mBottomControlsCoordinatorSupplier != null
                            && mBottomControlsCoordinatorSupplier.get()
                                    instanceof MisesBottomControlsCoordinator) {
                        ((MisesBottomControlsCoordinator) mBottomControlsCoordinatorSupplier.get())
                                .updateHomeButtonState();
                    }
                };
        HomepageManager.getInstance().addListener(mMisesHomepageStateListener);
    }
    ...
    public void enableBottomControls() {
        ...
        synchronized (mLock) {
            ...
            mBottomControls =
                    (MisesScrollingBottomViewResourceFrameLayout) bottomControlsStub.inflate();
            mTabGroupUi =
            ...
            mBottomControlsCoordinatorSupplier.set(
                    new MisesBottomControlsCoordinator(
                            mLayoutStateProviderSupplier,
    ...
    public void destroy() {
        super.destroy();
        HomepageManager.getInstance().removeListener(mMisesHomepageStateListener);
    ...
    private void recordNewTabClick() {
        ...
        //     if (mActivity instanceof MisesActivity) {
        //         MiscAndroidMetrics miscAndroidMetrics =
        //                 ((MisesActivity) mActivity).getMiscAndroidMetrics();
    ...
    protected void onOrientationChange(int newOrientation) {
        ...
        if (mActivity instanceof MisesActivity) {
            ((MisesActivity) mActivity).updateBottomSheetPosition(newOrientation);
        }
    }
    ...
    protected void updateBookmarkButtonStatus() {
        ...
        if (mBottomControlsCoordinatorSupplier.get() instanceof MisesBottomControlsCoordinator) {
            ((MisesBottomControlsCoordinator) mBottomControlsCoordinatorSupplier.get())
    ...
    private void setBottomToolbarVisible(boolean visible) {
        ...
        MisesMenuButtonCoordinator.setMenuFromBottom(isMenuFromBottom);

        if (mToolbar instanceof MisesTopToolbarCoordinator) {
            ((MisesTopToolbarCoordinator) mToolbar).onBottomToolbarVisibilityChanged(visible);
        }
        if (mBottomControlsCoordinatorSupplier.get() instanceof MisesBottomControlsCoordinator) {
            ((MisesBottomControlsCoordinator) mBottomControlsCoordinatorSupplier.get())
    ...
    private boolean isToolbarPhone() {
        assert (mToolbar instanceof MisesTopToolbarCoordinator);
        return mToolbar instanceof MisesTopToolbarCoordinator
                && ((MisesTopToolbarCoordinator) mToolbar).isToolbarPhone();
    }

    private ObservableSupplier<Integer> getConstraintsProxy() {
        if (mToolbar instanceof MisesTopToolbarCoordinator) {
            return ((MisesTopToolbarCoordinator) mToolbar).getConstraintsProxy();
        }
