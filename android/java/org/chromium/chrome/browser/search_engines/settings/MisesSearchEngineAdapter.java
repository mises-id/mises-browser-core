...
public class MisesSearchEngineAdapter extends SearchEngineAdapter {
    private static final String TAG = "MisesSearchEngineAdapter";
    ...
    public MisesSearchEngineAdapter(Context context, Profile profile) {
    ...
    public static String getDSEShortName(
        ...
        if (!readJavaPrefOnly) {
            ...
            // overwrite.
            if (MisesSearchEnginePrefHelper.getInstance().getFetchSEFromNative()) {
                // Set it for normal tab only
                setDSEPrefs(dseTemplateUrl, Profile.getLastUsedRegularProfile());
                MisesSearchEnginePrefHelper.getInstance().setFetchSEFromNative(false);
            }
        }
