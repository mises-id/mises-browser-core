...
#include "mises/components/brave_wallet/browser/solana_account_meta.h"
#include "mises/components/brave_wallet/browser/solana_instruction.h"
...
class TestEventsListener : public mojom::SolanaEventsListener {
 public:
  ...
  void AccountChangedEvent(
      const std::optional<std::string>& account) override {
...
TEST_F(SolanaProviderImplUnitTest, GetDeserializedMessage) {
  ...
  SolanaInstruction instruction(
      ...
      {SolanaAccountMeta(address, std::nullopt, true, true),
       SolanaAccountMeta(address, std::nullopt, false, true)},
