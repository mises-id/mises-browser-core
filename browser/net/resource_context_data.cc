...
void ResourceContextData::StartProxying(
  ...
  if (!self->request_handler_) {
    self->request_handler_ = std::make_unique<MisesRequestHandler>();
  }
...
// static
MisesProxyingWebSocket* ResourceContextData::StartProxyingWebSocket(
  ...
  auto proxy = std::make_unique<MisesProxyingWebSocket>(
...
void ResourceContextData::RemoveProxy(MisesProxyingURLLoaderFactory* proxy) {
  auto it = proxies_.find(proxy);
  DCHECK(it != proxies_.end());
  proxies_.erase(it);
}

void ResourceContextData::RemoveProxyWebSocket(MisesProxyingWebSocket* proxy) {
