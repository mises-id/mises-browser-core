...
MisesProxyingWebSocket::MisesProxyingWebSocket(
...
// static
MisesProxyingWebSocket* MisesProxyingWebSocket::ProxyWebSocket(
...
void MisesProxyingWebSocket::Start() {
  ...
  if (proxy_has_extra_headers()) {
    continuation = base::BindRepeating(
        &MisesProxyingWebSocket::ContinueToStartRequest,
        weak_factory_.GetWeakPtr());
  } else {
    continuation = base::BindRepeating(
        &MisesProxyingWebSocket::OnBeforeRequestComplete,
        weak_factory_.GetWeakPtr());
  }

  ctx_ = mises::MisesRequestInfo::MakeCTX(request_, process_id_,
...
MisesProxyingWebSocket::web_socket_factory() {
  return base::BindOnce(&MisesProxyingWebSocket::WebSocketFactoryRun,
                        base::Unretained(this));
}

mojo::Remote<network::mojom::WebSocketHandshakeClient>
MisesProxyingWebSocket::handshake_client() {
  return std::move(forwarding_handshake_client_);
}

bool MisesProxyingWebSocket::proxy_has_extra_headers() {
  return proxy_trusted_header_client_.is_bound();
}

void MisesProxyingWebSocket::WebSocketFactoryRun(
    const GURL& url,
...
void MisesProxyingWebSocket::OnOpeningHandshakeStarted(
...
void MisesProxyingWebSocket::ContinueToHeadersReceived() {
  auto continuation = base::BindRepeating(
      &MisesProxyingWebSocket::OnHeadersReceivedComplete,
      weak_factory_.GetWeakPtr());
  ctx_ = mises::MisesRequestInfo::MakeCTX(request_, process_id_,
                                          frame_tree_node_id_, request_id_,
                                          browser_context_, ctx_);
...
void MisesProxyingWebSocket::OnFailure(const std::string& message,
                                       int32_t net_error,
                                       int32_t response_code) {}

void MisesProxyingWebSocket::OnConnectionEstablished(
...
void MisesProxyingWebSocket::OnAuthRequired(
...
void MisesProxyingWebSocket::OnBeforeSendHeaders(
...
void MisesProxyingWebSocket::OnHeadersReceived(
...
void MisesProxyingWebSocket::OnBeforeRequestComplete(int error_code) {
  DCHECK(proxy_has_extra_headers() ||
  ...
  if (proxy_has_extra_headers()) {
    proxy_trusted_header_client_->OnBeforeSendHeaders(
        request_.headers,
        base::BindOnce(
            &MisesProxyingWebSocket::OnBeforeSendHeadersCompleteFromProxy,
            weak_factory_.GetWeakPtr()));
  } else {
...
void MisesProxyingWebSocket::OnBeforeSendHeadersCompleteFromProxy(
    int error_code,
  ...
  auto continuation = base::BindRepeating(
      &MisesProxyingWebSocket::OnBeforeSendHeadersComplete,
      weak_factory_.GetWeakPtr());
...
void MisesProxyingWebSocket::OnBeforeSendHeadersComplete(int error_code) {
  DCHECK(proxy_has_extra_headers() ||
         !receiver_as_handshake_client_.is_bound());
...
void MisesProxyingWebSocket::ContinueToStartRequest(int error_code) {
  if (error_code != net::OK) {
    OnError(error_code);
    return;
  }
  ...
  receiver_as_handshake_client_.set_disconnect_with_reason_handler(
      base::BindOnce(&MisesProxyingWebSocket::OnMojoConnectionError,
                     base::Unretained(this)));
}

void MisesProxyingWebSocket::OnHeadersReceivedCompleteFromProxy(
    int error_code,
...
void MisesProxyingWebSocket::OnHeadersReceivedComplete(int error_code) {
  if (error_code != net::OK) {
    OnError(error_code);
    return;
  }
  ...
  if (proxy_has_extra_headers()) {
    proxy_trusted_header_client_->OnHeadersReceived(
        ...
        remote_endpoint_,
        base::BindOnce(
            &MisesProxyingWebSocket::OnHeadersReceivedCompleteFromProxy,
            weak_factory_.GetWeakPtr()));
  } else {
...
void MisesProxyingWebSocket::PauseIncomingMethodCallProcessing() {
  receiver_as_handshake_client_.Pause();
  if (proxy_has_extra_headers())
    receiver_as_header_client_.Pause();
}

void MisesProxyingWebSocket::ResumeIncomingMethodCallProcessing() {
  receiver_as_handshake_client_.Resume();
  if (proxy_has_extra_headers())
    receiver_as_header_client_.Resume();
}

void MisesProxyingWebSocket::OnError(int error_code) {
  if (!is_done_) {
    is_done_ = true;
  }
...
// ResetWithReason
void MisesProxyingWebSocket::OnMojoConnectionError(
