...
class MisesIpfsClientUpdaterTest : public ExtensionBrowserTest {
 public:
  MisesIpfsClientUpdaterTest() = default;
  ...
  void SetComponentIdAndBase64PublicKeyForTest(
    ...
    ipfs::MisesIpfsClientUpdater::SetComponentIdAndBase64PublicKeyForTest(
  ...
  bool InstallIpfsClientUpdater(const std::string& extension_dir) {
    ...
    g_mises_browser_process->ipfs_client_updater()->OnComponentReady(
  ...
  void WaitForIpfsClientUpdaterThread() {
    scoped_refptr<base::ThreadTestHelper> io_helper(new base::ThreadTestHelper(
        g_mises_browser_process->ipfs_client_updater()->GetTaskRunner()));
  ...
  void SetIpfsExecutablePath(const base::FilePath& path) {
    g_mises_browser_process->ipfs_client_updater()->SetExecutablePath(path);
  }
};

// Load the Ipfs client updater extension and verify that it correctly
// installs the client.
IN_PROC_BROWSER_TEST_F(MisesIpfsClientUpdaterTest, IpfsClientInstalls) {
  ...
  base::FilePath executable_path =
      g_mises_browser_process->ipfs_client_updater()->GetExecutablePath();
  ASSERT_TRUE(PathExists(executable_path));
}

IN_PROC_BROWSER_TEST_F(MisesIpfsClientUpdaterTest, IpfsExecutableReady) {
