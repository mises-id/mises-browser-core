...
/**
 * Checks to make sure the src/chrome/VERSION matches mises-browser-core's package.json version
 */
const checkVersionsMatch = () => {
  ...
  const misesVersionFromChromeFile = `${found[2]}.${found[3]}.${found[4]}`
  if (misesVersionFromChromeFile !== config.misesVersion) {
    // Only a warning. The CI environment will choose to proceed or not within its own script.
    console.warn(`Version files do not match!\nsrc/chrome/VERSION: ${misesVersionFromChromeFile}\nmises-browser-core package.json version: ${config.misesVersion}`)
  }
}
