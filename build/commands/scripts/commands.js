...
// https://github.com/mises/mises-browser/issues/13724
...
program
  ...
  .option('--android_override_version_code <android_override_version_code>', 'Android version code')
  ...
  .option('--mises_google_api_key <mises_google_api_key>')
  .option('--mises_google_api_endpoint <mises_google_api_endpoint>')
  .option('--mises_infura_project_id <mises_infura_project_id>')
  .option('--mises_zero_ex_api_key <mises_zero_ex_api_key>')
  ...
  .option('--mises_safetynet_api_key <mises_safetynet_api_key>')
...
program
  ...
  .option('--skip_signing', 'skip signing dmg/mises_installer.exe')
...
program
  ...
  .option('--disable_mises_extension', 'disable loading the Mises extension')
  .option('--disable_mises_rewards_extension', 'disable loading the Mises Rewards extension')
  ...
  .option('--enable_mises_update', 'enable mises update')
  ...
  // See https://github.com/mises/mises-browser/wiki/Rewards#flags for more information
  .option('--rewards [options]', 'options for rewards')
  .option('--mises_ads_testing', 'ads testing')
  .option('--mises_ads_production', 'ads production')
  .option('--mises_ads_staging', 'ads staging')
  .option('--mises_ads_debug', 'ads debug')
  .option('--single_process', 'use a single process')
  .option('--output_path [pathname]', 'use the Mises binary located at [pathname]')
  .arguments('[build_config]')
  .action(start.bind(null, parsedArgs.unknown))

program
  .command('pull_l10n')
  .option('--extension <extension>', 'Scope this command to localize a Mises extension such as greaselion')
