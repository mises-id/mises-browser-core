...
function maybeInstallDepotTools(options = config.defaultOptions) {
  options.cwd = config.misesCoreDir
...
function shouldUpdateChromium(latestSyncInfo, expectedSyncInfo) {
  ...
  if (needsUpdate) {
    ...
    console.log(`Chromium repo ${chalk.blue.bold('needs sync')}.\n  target is ${
        ...
        JSON.stringify(latestSyncInfo, null, 4)} expected  sync is ${
...
function syncChromium(program) {
  ...
  const latestSyncInfoFilePath =
      path.join(config.rootDir, '.mises_latest_successful_sync.json')
  ...
  if (program.delete_unused_deps) {
    if (util.isGitExclusionExists(config.srcDir, 'mises/')) {
      args.push('-D')
    } else if (!config.isCI) {
      Log.warn(
          '--delete_unused_deps is ignored because sync has not yet added ' +
          'the exclusion for the src/mises/ directory, likely because sync ' +
  ...
  util.addGitExclusion(config.srcDir, 'mises/')
...
function syncMises(program) {
  ...
  util.runGClient(
      args, {cwd: config.misesCoreDir},
      path.join(config.misesCoreDir, '.mises_gclient'))
}
...
async function RunCommand() {
  ...
  if (syncWithForce || !didSyncChromium || program.delete_unused_deps) {
    // If no Chromium sync was done, run sync inside `mises` to sync Mises DEPS.
    syncMises(program)
  }
...
Log.progress('Mises Browser Sync starting')
RunCommand()
.then(() => {
  Log.progress('Mises Browser Sync complete')
})
.catch((err) => {
  Log.error('Mises Browser Sync ERROR:')
  console.error(err)
  process.exit(1)
})
