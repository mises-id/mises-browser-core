...
from signing_helper import GetMisesSigningConfig
...
# src/out/Release/Mises_Browser_CHANNEL_Packaging), the signing directory is
# relative to that
...
def create_config(config_args, development, mac_provisioning_profile=None):
    ...
    config_class = GetMisesSigningConfig(config_class, mac_provisioning_profile)
    return config_class(*config_args)


def NotarizeMisesDmgPkg(paths, config, dmg, pkg, outdir, signed):
    """
    Notarize Mises .dmg and .pkg files.
    """
    uuids_to_path_map = {}
    for dist in config.distributions:
        ...
        for result in notarize.wait_for_results(
            ...
            mises_path = uuids_to_path_map[result]
            notarize.staple(mises_path)
...
def main():
    ...
    rc = NotarizeMisesDmgPkg(paths, config, args.dmg, args.pkg, args.outdir, args.signed)
    return rc
