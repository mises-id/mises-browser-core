...
def _OverrideVersionKey(plist, mises_version):
    """ `minor.build` version string is used for update.
    When we begin to use the Major version component, Mises version string will
    ...
    version_values = mises_version.split('.')
...
def Main():
    ...
    parser.add_argument('--mises_channel', dest='mises_channel', action='store',
        default=None, help='Channel (beta, dev, nightly)')
    parser.add_argument('--mises_product_dir_name', dest='mises_product_dir_name',
        action='store', default=None,
        help='Product directory name')
    parser.add_argument('--mises_eddsa_key', dest='mises_eddsa_key', action='store',
        default=None, help='Public EdDSA key for update')
    parser.add_argument('--mises_version', dest='mises_version', action='store',
        default=None, help='mises version string')
    ...
    if args.skip_signing and args.mises_channel != "":
        plist['KSChannelID'] = args.mises_channel
    elif 'KSChannelID' in plist:
        # 'KSChannelID' is set at _modify_plists() of modification.py only
        # during signing
        del plist['KSChannelID']

    plist['CrProductDirName'] = args.mises_product_dir_name

    if args.mises_eddsa_key:
        plist['SUPublicEDKey'] = args.mises_eddsa_key

    _OverrideVersionKey(plist, args.mises_version)
