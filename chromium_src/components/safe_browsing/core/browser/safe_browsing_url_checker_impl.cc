...
UnsafeResource SafeBrowsingUrlCheckerImpl::MakeUnsafeResource(
  ...
  resource.threat_source = is_from_real_time_check
    ...
    LOG(INFO) << "Cg SafeBrowsingUrlCheckerImpl::MakeUnsafeResource -1 url: "
  << url
  << ", threat_type: " << (threat_type)
  << ", original_url: " << resource.original_url
  << ", is_subresource: " << resource.is_subresource
  << ", original_url: " << resource.original_url
  ;
  return resource;
}
...
void SafeBrowsingUrlCheckerImpl::OnUrlResult(const GURL& url,
                                             ...
                                             const ThreatMetadata& metadata,
                                             bool is_from_real_time_check) {
  LOG(INFO) << "Cg SafeBrowsingUrlCheckerImpl::OnUrlResult -1 url: "
  << url
  << ", threat_type: " << (threat_type)
  << ", state_: " << state_
  ;
  ...
  if (threat_type == SB_THREAT_TYPE_SAFE ||
    ...
    LOG(INFO) << "Cg SafeBrowsingUrlCheckerImpl::OnUrlResult -2 url: "
  << url
  << ", state_: " << (state_)
  ;
  if (is_prefetch) {
  ...
  UnsafeResource resource =
    ...
    LOG(INFO) << "Cg SafeBrowsingUrlCheckerImpl::OnUrlResult -3 url: "
  << url
  << ", state_: " << (state_)
  ;
  ...
  url_checker_delegate_->StartDisplayingBlockingPageHelper(
      ...
      request_destination_ == network::mojom::RequestDestination::kDocument,
      has_user_gesture_);
          LOG(INFO) << "Cg SafeBrowsingUrlCheckerImpl::OnUrlResult -4 url: "
  << url
  << ", state_: " << (state_)
  ;
}
