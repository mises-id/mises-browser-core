...
#include "mises/components/safe_browsing/android/safe_browsing_mises.h"
...
void SafeBrowsingApiHandlerBridge::StartURLCheck(
  ...
  LOG(INFO) << "Cg SafeBrowsingApiHandlerBridge::StartURLCheck(com_safe_android) -1.2";
  //scheme is http or https
  DCHECK_CURRENTLY_ON(BrowserThread::IO);
  LOG(INFO) << "Cg SafeBrowsingApiHandlerBridge::StartURLCheck(com_safe_android) -1.31";
  if (url.SchemeIsHTTPOrHTTPS()){
    //domain
    LOG(INFO) << "Cg SafeBrowsingApiHandlerBridge::StartURLCheck(com_safe_android) -1.32";
    raw_ptr<SafeBrowsingMises> mises_ = new SafeBrowsingMises(url_loader_factory);
     LOG(INFO) << "Cg SafeBrowsingApiHandlerBridge::StartURLCheck(com_safe_android) -1.4";
