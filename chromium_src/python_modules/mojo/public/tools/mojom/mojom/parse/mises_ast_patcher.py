  ...
  if brave_definition.attribute_list:
    for attribute in brave_definition.attribute_list:
      if attribute.key == 'MisesAdd':
        return _DEFINITION_ADD
      elif attribute.key == 'MisesExtend':
        return _DEFINITION_EXTEND
  raise ValueError("Definition should have [MisesAdd] or [MisesExtend] attribute: %s" %
...
def PatchMojomAst(mojom_abspath, ast, enabled_features):
  ...
  To be more strict with patching, attributes [MisesAdd] or [MisesExtend] should be used.
  """
  ...
  chromium_src_abspath = os.path.join(chromium_original_dir, 'mises', 'chromium_src')
  if not os.path.isdir(chromium_src_abspath):
    raise RuntimeError("Could not find mises/chromium_src. %s is not a dir" % chromium_src_abspath)
