...
bool ProcessMap::Insert(const ExtensionId& extension_id, int process_id) {
  //LOG(INFO) << "ProcessMap::Insert(" << extension_id << ", " << process_id << ")";
  auto* extensions = base::FindOrNull(items_, process_id);
  if (extensions) {
    extensions->push_back(extension_id);
    return false;
  }
  std::vector<ExtensionId> new_extensions;
  new_extensions.push_back(extension_id);
  return items_.emplace(process_id, new_extensions).second;
}

int ProcessMap::Remove(int process_id) {
  //LOG(INFO) << "ProcessMap::Remove(" <<  process_id << ")";
  return items_.erase(process_id);
}

bool ProcessMap::Contains(const ExtensionId& extension_id_in,
                          int process_id) const {
  //LOG(INFO) << "ProcessMap::Contains(" << extension_id_in << "," << process_id << ")";
  auto* extensions = base::FindOrNull(items_, process_id);
  return extensions && base::Contains(*extensions, extension_id_in);
}

bool ProcessMap::Contains(int process_id) const {
  //LOG(INFO) << "ProcessMap::Contains(" <<  process_id << ")";
  return base::Contains(items_, process_id);
}
bool ProcessMap::ContainsExtension(const ExtensionId& extension_id_in) const {
  for (const auto& [key, value] : items_) {
    if (base::Contains(value, extension_id_in)) {
      return true;
    }
  }
  return false;
...
const Extension* ProcessMap::GetEnabledExtensionByProcessID(
    int process_id) const {
  //LOG(INFO) << "ProcessMap::GetEnabledExtensionByProcessID(" <<  process_id << ")";
  auto* extensions = base::FindOrNull(items_, process_id);
  return (extensions &&  extensions->size() > 0) ? ExtensionRegistry::Get(browser_context_)
                            ->enabled_extensions()
                            .GetByID((*extensions)[0])
                      : nullptr;
}

std::optional<ExtensionId> ProcessMap::GetExtensionIdForProcess(
    int process_id) const {
  //LOG(INFO) << "ProcessMap::GetExtensionIdForProcess(" <<  process_id << ")";
  auto* extensions = base::FindOrNull(items_, process_id);
  if (!extensions || extensions->size() == 0) {
    return std::nullopt;
  }
  return base::OptionalFromPtr(&(*extensions)[0]);
}

bool ProcessMap::IsPrivilegedExtensionProcess(const Extension& extension,
                                              int process_id) {
  //LOG(INFO) << "ProcessMap::IsPrivilegedExtensionProcess(" << extension.id() << "," << process_id << ")";
...
bool ProcessMap::CanProcessHostContextType(
  ...
  // if (extension) {
  //   LOG(INFO) << "ProcessMap::CanProcessHostContextType(" << extension->id() << "," << process_id << "," << context_type << ")";
  // } else {
  //   LOG(INFO) << "ProcessMap::CanProcessHostContextType(" << process_id << "," << context_type << ")";
  // }
...
mojom::ContextType ProcessMap::GetMostLikelyContextType(
  ...
  //LOG(INFO) << "ProcessMap::GetMostLikelyContextType(" <<  process_id << ")";
...
bool ProcessMap::IsIsolateExtensionsEnabled() {
#if BUILDFLAG(IS_ANDROID)
  return preferences::features::IsMisesAllowExtensionIsolation();
#else
  return true;
#endif
}
