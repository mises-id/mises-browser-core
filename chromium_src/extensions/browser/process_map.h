...
class ProcessMap : public KeyedService {
 public:
  ...
  bool ContainsExtension(const ExtensionId& extension_id) const;
  ...
  static bool IsIsolateExtensionsEnabled();//for one process host one extension

 private:
  using ProcessId = int;

  base::flat_map<ProcessId, std::vector<ExtensionId>> items_;
