...
int GetIndexOfExistingTab(Browser* browser, const NavigateParams& params) {
  ...
  int start_index = std::max(0, GetActiveTabIndexAndroid());
  int tab_count = GetTabsCountAndroid();
  for (int i = 0; i < tab_count; ++i) {
    int tab_index = (start_index + i) % tab_count;
    content::WebContents* tab = GetWebContentsAtAndroid(tab_index);
...
void ShowSingletonTabOverwritingNTP(NavigateParams* params) {
  ...
  content::WebContents* contents = GetActiveWebContentsAndroid();
  if (contents) {
    ...
    if (contents_url == chrome::kChromeUINewTabURL ||
      ...
      if (tab_index < 0) {
        ...
        params->switch_to_singleton_tab = GetWebContentsAtAndroid(tab_index);
      }
    }
  }
  Navigate(params);
