...
bool ChromePermissionsClient::MisesCanBypassEmbeddingOriginCheck(
...
ChromePermissionsClient::MaybeCreateMessageUI(
  ...
  if (requests.size() > 0) {
    ...
    if (request_type == permissions::RequestType::kMisesEthereum ||
        request_type == permissions::RequestType::kMisesSolana) {
      if (request_type == permissions::RequestType::kMisesSolana) {
