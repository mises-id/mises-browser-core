...
#include "mises/app/mises_command_line_helper.h"
...
MisesCommandLineHelper::MisesCommandLineHelper(base::CommandLine* command_line)
    : command_line_(*command_line) {
  Parse();
}

const std::unordered_set<std::string>&
MisesCommandLineHelper::enabled_features() const {
  return enabled_features_;
}

const std::unordered_set<std::string>&
MisesCommandLineHelper::disabled_features() const {
  return disabled_features_;
}

void MisesCommandLineHelper::ParseCSV(
...
void MisesCommandLineHelper::Parse() {
...
void MisesCommandLineHelper::AppendSwitch(const char* switch_key) {
  if (!command_line_->HasSwitch(switch_key)) {
    command_line_->AppendSwitch(switch_key);
  }
}

void MisesCommandLineHelper::AppendFeatures(
...
void MisesCommandLineHelper::AppendCSV(
...
void MisesCommandLineHelper::AppendSwitchASCII(const char* switch_key,
                                               const char* value) {
  if (!command_line_->HasSwitch(switch_key)) {
