...
def to_bytes(data: Sequence[int]) -> bytes:
 
  return bytes(data)

def words_to_binary(words: Iterable[str]) -> bytes:
  """Generates C++ code from a word list"""
  dafsa = to_dafsa(words)
  for fun in (reverse, join_suffixes, reverse, join_suffixes, join_labels):
    dafsa = fun(dafsa)
  return to_bytes(encode(dafsa))
...
def main() -> int:
  ...
  parser.add_argument('--reverse', action='store_const', const=True,
                      default=False)
  parser.add_argument('--binary', action='store_const', const=True,
                      default=False)
  ...
  if args.binary:
    outfile_mode = 'wb'
  with open(args.outfile, outfile_mode) as outfile:
    if args.binary:
      outfile.write(words_to_binary(parse_gperf(args.infile, args.reverse)))
    else:
      outfile.write(words_to_cxx(parse_gperf(args.infile, args.reverse)))
  return 0
