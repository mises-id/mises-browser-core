...
export class Mnemonic {
  static async generateWallet(
    ...
    const mnemonic = await Mnemonic.generateSeed(strength);
  ...
  static async generateSeed(strength: number = 128): Promise<string> {
    if (strength % 32 !== 0) {
      throw new TypeError("invalid entropy");
    }
    let bytes = new Uint8Array(strength / 8);
    bytes = await crypto.getRandomValues(bytes);
