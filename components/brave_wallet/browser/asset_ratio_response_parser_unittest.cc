...
#include "mises/components/brave_wallet/browser/asset_ratio_response_parser.h"
...
TEST(AssetRatioResponseParserUnitTest, ParseAssetPriceHistory) {
  ...
  base::Time date = base::Time::FromMillisecondsSinceUnixEpoch(values[0]->date.InMilliseconds());
  ...
  base::Time date1 = base::Time::FromMillisecondsSinceUnixEpoch(values[1]->date.InMilliseconds());
