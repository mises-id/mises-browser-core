...
bool ResolveCall<ResultType>::SetNoResult(const std::string& chain_id) {
...
template <class ResultType>
bool ResolveCall<ResultType>::SetResult(const std::string& chain_id,
                                           ResultType result) {
  DCHECK(!responses_.count(chain_id));
...
template <class ResultType>
bool ResolveCall<ResultType>::SetError(const std::string& chain_id,
...
template <class ResultType>
void ResolveCall<ResultType>::AddCallback(CallbackType cb) {
  callbacks_.push_back(std::move(cb));
}

template <class ResultType>
typename ResolveCall<ResultType>::Response*
ResolveCall<ResultType>::GetEffectiveResponse() {
...
template <class ResultType>
bool ResolveCall<ResultType>::MaybeResolveCallbacks() {
...
std::vector<std::string> ResolveCalls<KeyType, ResultType>::GetChains()
    const {
  return {mojom::kMainnetChainId};
}

template <class KeyType, class ResultType>
bool ResolveCalls<KeyType, ResultType>::HasCall(const KeyType& key) {
  return calls_.count(key) > 0;
}

template <class KeyType, class ResultType>
void ResolveCalls<KeyType, ResultType>::AddCallback(const KeyType& key,
                                                       CallbackType callback) {
  calls_[key].AddCallback(std::move(callback));
}

template <class KeyType, class ResultType>
void ResolveCalls<KeyType, ResultType>::SetNoResult(
    const KeyType& key,
...
template <class KeyType, class ResultType>
void ResolveCalls<KeyType, ResultType>::SetResult(
    const KeyType& key,
    const std::string& chain_id,
    ResultType result) {
  auto call = calls_.find(key);
...
template <class KeyType, class ResultType>
void ResolveCalls<KeyType, ResultType>::SetError(const KeyType& key,
                                                    const std::string& chain_id,
                                                    mojom::ProviderError error,
                                                    std::string error_message) {
  auto call = calls_.find(key);
...
template class ResolveCalls<std::string, std::string>;
template class ResolveCalls<WalletAddressKey, std::string>;
template class ResolveCalls<std::string, GURL>;
