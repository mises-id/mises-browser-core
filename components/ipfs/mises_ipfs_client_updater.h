...
class MisesIpfsClientUpdaterTest;

using mises_component_updater::MisesComponent;
...
class MisesIpfsClientUpdater : public MisesComponent,
                               public MisesComponent::ComponentObserver {
 public:
  class Observer : public base::CheckedObserver {
   public:
    virtual void OnExecutableReady(const base::FilePath& path) = 0;
    virtual void OnInstallationEvent(
        MisesComponent::ComponentObserver::Events event) = 0;

   protected:
    ~Observer() override = default;
  };

  explicit MisesIpfsClientUpdater(MisesComponent::Delegate* delegate,
                                  const base::FilePath& user_data_dir);
  MisesIpfsClientUpdater(const MisesIpfsClientUpdater&) = delete;
  MisesIpfsClientUpdater& operator=(const MisesIpfsClientUpdater&) = delete;
  ~MisesIpfsClientUpdater() override;
 ...
 protected:
  ...
  // MisesComponent::ComponentObserver
  void OnEvent(Events event, const std::string& id) override;

 private:
  friend class ::MisesIpfsClientUpdaterTest;
  ...
  base::ScopedObservation<MisesComponent, MisesComponent::ComponentObserver>
      updater_observer_{this};

  base::WeakPtrFactory<MisesIpfsClientUpdater> weak_ptr_factory_;
};

// Creates the MisesIpfsClientUpdater
