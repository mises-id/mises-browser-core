...
public class MisesVariationsSeedFetcher extends VariationsSeedFetcher {
    ...
    private static final String TAG = "MisesVariations";

    public static VariationsSeedFetcher get() {
        synchronized (sLock) {
            if (sInstance == null) {
                sInstance = new MisesVariationsSeedFetcher();
            }
            return sInstance;
        }
    }
    ...
    @VisibleForTesting
    public MisesVariationsSeedFetcher() {
        super();
    }
    ...
    protected String getConnectionString(SeedFetchParameters params) {
        ...
        // Replace Chromium URL with Mises URL.
        if (urlString.indexOf(DEFAULT_VARIATIONS_SERVER_URL) != -1) {
            urlString = urlString.replaceFirst(
                    DEFAULT_VARIATIONS_SERVER_URL, MisesVariationsConfig.VARIATIONS_SERVER_URL);
            ...
            urlString = urlString.replaceFirst(DEFAULT_FAST_VARIATIONS_SERVER_URL,
                    MisesVariationsConfig.VARIATIONS_SERVER_URL);
        } else {
            Log.e(TAG, "Cannot replace seed URL to fetch variations: %s", urlString);
            urlString = "";
        }

        Log.i(TAG, "Fetching variations from: %s", urlString.replaceFirst("https://", ""));
        return urlString;
    }
}
