...
#include "mises/components/ephemeral_storage/ephemeral_storage_service.h"
...
#include "mises/components/ephemeral_storage/ephemeral_storage_pref_names.h"
#include "mises/components/ephemeral_storage/url_storage_checker.h"
...
EphemeralStorageService::EphemeralStorageService(
  ...
  tld_ephemeral_area_keep_alive_ = base::Seconds(
      net::features::kMisesEphemeralStorageKeepAliveTimeInSeconds.Get());
...
void EphemeralStorageService::TLDEphemeralLifetimeDestroyed(
  ...
  if (base::FeatureList::IsEnabled(
          net::features::kMisesEphemeralStorageKeepAlive)) {
...
bool EphemeralStorageService::FirstPartyStorageAreaNotInUse(
  ...
  if (host_content_settings_map_->GetContentSetting(
          url, url, ContentSettingsType::MISES_REMEMBER_1P_STORAGE) !=
...
void EphemeralStorageService::ScheduleFirstPartyStorageAreasCleanupOnStartup() {
  ...
  first_party_storage_areas_startup_cleanup_timer_.Start(
      ...
      base::Seconds(
          net::features::
              kMisesForgetFirstPartyStorageStartupCleanupDelayInSeconds.Get()),
      base::BindOnce(
